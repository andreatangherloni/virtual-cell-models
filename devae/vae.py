...
class AttentionEnhancedDualEncoderVAE(nn.Module):
    ...
    2. Attention-based delta predictor
    3. Decoder with optional FiLM conditioning
    4. Attention-modulated count head
    ...
    def predict_delta(
        ...
        """
        Predict perturbation effect delta.
        ...
        Returns:
            delta: [B, D] - perturbation effect
            gene_attention: [B, G] - gene attention scores (if using attention)
            attn_weights: [B, H, G] - raw attention weights (if using attention)
        """
        ...
        if self.cfg.use_attention:
            delta, gene_attention, attn_weights = self.delta_predictor(
    ...
    def predict_with_neighbor_mixing(
        ...
        Returns:
            ...
            attention_mixed: [B, G] - mixed attention scores (if using attention)
        """
        device = z_c.device
        B = z_c.size(0)
        ...
        for i in range(K):
            ...
            if self.cfg.use_attention:
                delta, attn, _ = self.delta_predictor(
        ...
        # Average attentions if available
        if attentions:
            attentions_stacked = torch.stack(attentions, dim=1)  # [B, K, G]
            attention_mixed = (weights.unsqueeze(-1) * attentions_stacked).sum(dim=1)  # [B, G]
        else:
            attention_mixed = None
        
        # Decode
        x_pred = self.decode(z_c + delta_mixed, context)
        
        return x_pred, attention_mixed
    ...
    def counts_rate(
        ...
        if self.count_head is None:
            return None
        return self.count_head(z, gene_attention)
    ...
    def forward(self, batch: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        delta_raw, gene_attention, attn_weights = self.predict_delta(z_c, gene_idx, context)
        ...
        rates_c = self.counts_rate(z_c, gene_attention)
        rates_p = self.counts_rate(z_p, gene_attention)
        rates_pred = self.counts_rate(z_c + delta, gene_attention)
